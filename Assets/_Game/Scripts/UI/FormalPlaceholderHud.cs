...
namespace WasteCity.UI
{
    public sealed class FormalPlaceholderHud : MonoBehaviour
    {
        ...
        private void OnGUI()
        {
            string resources = economy == null ? string.Empty : $"基础：铁 {economy.Inventory.Get(ResourceIds.Iron)} 能晶 {economy.Inventory.Get(ResourceIds.EnergyCrystal)} 石 {economy.Inventory.Get(ResourceIds.Stone)} 生物质 {economy.Inventory.Get(ResourceIds.Biomass)} 水 {economy.Inventory.Get(ResourceIds.Water)}\n科技：合金 {economy.Inventory.Get(ResourceIds.Alloy)} 弹药 {economy.Inventory.Get(ResourceIds.Ammunition)}｜修仙：灵铁 {economy.Inventory.Get(ResourceIds.SpiritIron)} 飞剑 {economy.Inventory.Get(ResourceIds.FlyingSword)}\n血肉：骨钢 {economy.Inventory.Get(ResourceIds.BoneSteel)} 浓缩液 {economy.Inventory.Get(ResourceIds.BiomassConcentrate)} 生物武器 {economy.Inventory.Get(ResourceIds.BiologicalWeapon)}\n灵能：共振金属 {economy.Inventory.Get(ResourceIds.ResonanceMetal)} 增幅器 {economy.Inventory.Get(ResourceIds.PsionicAmplifier)}";
