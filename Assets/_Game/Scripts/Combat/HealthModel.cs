...
namespace WasteCity.Combat
{
    public sealed class HealthModel
    {
        ...
        public int ApplyTrueDamage(int rawDamage)
        {
            if(IsDead)return 0;int amount=Math.Min(Current,Math.Max(0,rawDamage));if(amount<=0)return 0;Current-=amount;Damaged?.Invoke(amount);if(IsDead)Died?.Invoke();return amount;
        }
