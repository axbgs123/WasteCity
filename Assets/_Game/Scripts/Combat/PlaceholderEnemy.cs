...
namespace WasteCity.Combat
{
    ...
    public sealed class PlaceholderEnemy : MonoBehaviour
    {
        ...
        private bool heavy;
        public void Configure(HealthComponent targetHealth, Transform target, bool isHeavy, ResourceInventory inventory)
        {
            heavy = isHeavy; lootInventory = inventory; health = GetComponent<HealthComponent>(); health.Configure(heavy ? 240 : 70, heavy ? ArmorType.Heavy : ArmorType.Light);
            cityHealth = targetHealth; city = target; health.Value.Died += OnDied;
        }
        private void OnDied(){lootInventory?.Add(ResourceIds.Biomass,heavy?3:1);Destroy(gameObject);}
        private void Update()
        {
            ...
            foreach(var building in Object.FindObjectsOfType<BuildingRuntime>()){float distance=Vector2.Distance(transform.position,building.transform.position);if(distance<best){best=distance;targetHealth=building.Health;targetTransform=building.transform;}}
