...
{
    [Serializable] public sealed class EnemySnapshot { public int archetype,quality,waveTrigger,health,shield;public float x,y;public bool controlled;public BossEncounterSnapshot boss; }
    public sealed class FormalCombatController : MonoBehaviour
    {
        ...
        private PlaceholderEnemy Spawn(EnemyArchetype archetype,int threshold,int slot,Vector2? restoredPosition=null,int restoredHealth=-1,int restoredShield=0,BossEncounterSnapshot restoredBoss=null,int restoredQuality=-1,bool restoredControlled=false)
        {
        ...
        public EnemySnapshot[] CaptureEnemies(){var values=UnityEngine.Object.FindObjectsOfType<PlaceholderEnemy>();var result=new EnemySnapshot[values.Length];for(int i=0;i<values.Length;i++){var enemy=values[i];var health=enemy.GetComponent<HealthComponent>().Value;result[i]=new EnemySnapshot{archetype=(int)enemy.Definition.Archetype,quality=(int)enemy.Quality,waveTrigger=enemy.WaveTrigger,health=health.Current,shield=health.Shield,x=enemy.transform.position.x,y=enemy.transform.position.y,controlled=enemy.IsControlled,boss=enemy.GetComponent<PlaceholderBossEncounter>()?.Capture()};}return result;}
