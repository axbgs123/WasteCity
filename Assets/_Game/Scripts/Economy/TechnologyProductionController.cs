...
namespace WasteCity.Economy
{
    public sealed class TechnologyProductionController : MonoBehaviour
    {
        ...
        private void Update(){int smelters=0,assemblers=0;foreach(var runtime in Object.FindObjectsOfType<BuildingRuntime>()){if(!runtime.Construction.IsComplete||!runtime.HasLogistics)continue;int units=Mathf.RoundToInt(localHaste?.MultiplierFor(runtime)??1f);if(runtime.Definition.Id.Value=="core.building.smelter")smelters+=units;if(runtime.Definition.Id.Value=="core.building.assembler")assemblers+=units;}smelter.Tick(Time.deltaTime,economy.Inventory,legacyEffects?.Model?.ProductionUnits(smelters)??smelters);assembler.Tick(Time.deltaTime,economy.Inventory,legacyEffects?.Model?.ProductionUnits(assemblers)??assemblers);}
    }
}
