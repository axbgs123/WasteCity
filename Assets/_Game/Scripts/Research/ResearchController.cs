...
namespace WasteCity.Research
{
    public sealed class ResearchController : MonoBehaviour
    {
        ...
        {
            if (!visible) return;
            string status = buildings.CompletedCount("core.building.research-station")<=0?"锁定：需要已完成研究站":Model.Active == null ? $"已完成 {Model.CompletedCount}" : $"研究中：{Model.Active.Name} · {Model.Remaining:0.0}s";
            GUI.Box(new Rect(Screen.width - 430f, Screen.height - 215f, 410f, 190f), "四路线研究占位面板 [K]\n" + status + "\n\n[5] 自动机械（铁）\n[6] 灵气感知（能晶）\n[7] 适应组织（生物质）\n[8] 意识共鸣（水）");
        }
    }
}
