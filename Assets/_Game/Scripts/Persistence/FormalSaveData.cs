...
namespace WasteCity.Persistence
{
    ...
    { public int schema=14, worldSeed, iron, energyCrystal, stone, biomass, water, alloy, ammunition, population=100, populationCapacity=150, civilizationLevel=1, cityHealth=2000, day=1, foresightFlashedDay, hastePoolDay=1, hasteTargetX=-1, hasteTargetY=-1, cityMode, guidanceStage; public float cityX, cityY, observation, secondsIntoDay, hasteRemaining=60f, territoryProgress, researchRemaining, deploymentRemaining; public bool hasteActive, territoryActivated; public string legacyPathId, activeResearchId; public string[] completedResearchIds; public BuildingSnapshot[] buildings; public bool[] rescuedSites, worldRevealed; public int[] worldResourceAmounts, territoryLocalResources; public Legacy.SpatialTemplateEntry[] spatialTemplate; public WaveDirectorSnapshot wave; public EnemySnapshot[] enemies; }
    public static class FormalSaveCodec
    {
        ...
        { if(string.IsNullOrWhiteSpace(json)) return null; try { var d=JsonUtility.FromJson<FormalSaveData>(json); return d!=null&&(d.schema>=1&&d.schema<=14)?d:null; } catch { return null; } }
    }
}
