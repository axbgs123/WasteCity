...
namespace WasteCity.Persistence
{
    public sealed class FormalSaveController : MonoBehaviour
    {
        ...
        public FormalSaveData CaptureComplete(){var data=Capture();data.puppetProgress=friendlyUnits.Fabrication.Progress;data.puppets=friendlyUnits.Capture();return data;}
        public bool Load(){var d=Read(SavePath)??Read(SavePath+".bak");return d!=null&&ApplyComplete(d,false);}
        public bool ApplyComplete(FormalSaveData data,bool preserveObservation){bool applied=Apply(data,preserveObservation);if(applied&&data.schema>=22)friendlyUnits.Restore(data.puppetProgress,data.puppets);return applied;}
