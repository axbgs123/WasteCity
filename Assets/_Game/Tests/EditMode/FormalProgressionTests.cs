...
namespace WasteCity.Tests
{
    public sealed class FormalProgressionTests
    {
        ...
        [Test] public void AdvancementSequenceReachesResultsAndCanContinue(){var m=new AdvancementSequenceModel();Assert.That(m.Start(),Is.True);m.Tick(20);Assert.That(m.Stage,Is.EqualTo(AdvancementSequenceStage.Results));Assert.That(m.Continue(),Is.True);Assert.That(m.Stage,Is.EqualTo(AdvancementSequenceStage.Continued));}
        [Test] public void AdvancementSequenceCanBeRestored(){var m=new AdvancementSequenceModel();m.Restore((int)AdvancementSequenceStage.Warning,2.25f);Assert.That(m.Stage,Is.EqualTo(AdvancementSequenceStage.Warning));Assert.That(m.Remaining,Is.EqualTo(2.25f));}
    }
}
