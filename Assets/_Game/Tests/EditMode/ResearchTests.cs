...
namespace WasteCity.Tests
{
    public sealed class ResearchTests
    {
        ...
        [Test] public void PreviewResearchCannotStartOrSpend(){var preview=ResearchCatalog.Find("core.research.ballistics");var inventory=new ResourceInventory(200);inventory.Add(ResourceIds.Iron,12);inventory.Add(ResourceIds.Alloy,10);var model=new ResearchModel();model.Restore(new[]{"core.research.automated-machinery"},null,0);Assert.That(preview.ReleaseState,Is.EqualTo(ResearchReleaseState.PreviewOnly));Assert.That(model.Start(preview,inventory),Is.False);Assert.That(inventory.Get(ResourceIds.Iron),Is.EqualTo(12));Assert.That(inventory.Get(ResourceIds.Alloy),Is.EqualTo(10));}
        [Test] public void ExtendedResearchNodesRoundTripWithPreviewActiveFrozen(){var model=new ResearchModel();model.Restore(new[]{"core.research.alloy-armor","core.research.collective-consciousness"},"core.research.ballistics",17);Assert.That(model.IsCompleted(ResearchCatalog.Find("core.research.alloy-armor").Id),Is.True);Assert.That(model.Active,Is.Null);Assert.That(model.MissingActiveResearchId,Is.EqualTo("core.research.ballistics"));Assert.That(model.Remaining,Is.EqualTo(17));Assert.That(model.CaptureForPersistence().ActiveResearchId,Is.EqualTo("core.research.ballistics"));}
