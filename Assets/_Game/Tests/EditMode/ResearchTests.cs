...
namespace WasteCity.Tests
{
    public sealed class ResearchTests
    {
        ...
        [Test] public void ReleasedBallisticsStartsAndSpends(){var released=ResearchCatalog.Find("core.research.ballistics");var inventory=new ResourceInventory(200);inventory.Add(ResourceIds.Iron,12);inventory.Add(ResourceIds.Alloy,10);var model=new ResearchModel();model.Restore(new[]{"core.research.automated-machinery"},null,0);Assert.That(released.ReleaseState,Is.EqualTo(ResearchReleaseState.Researchable));Assert.That(model.Start(released,inventory),Is.True);Assert.That(inventory.Get(ResourceIds.Iron),Is.Zero);Assert.That(inventory.Get(ResourceIds.Alloy),Is.Zero);}
        [Test] public void ReleasedResearchNodesRoundTripWithActiveProgress(){var model=new ResearchModel();model.Restore(new[]{"core.research.alloy-armor","core.research.collective-consciousness"},"core.research.ballistics",17);Assert.That(model.IsCompleted(ResearchCatalog.Find("core.research.alloy-armor").Id),Is.True);Assert.That(model.Active.Id.Value,Is.EqualTo("core.research.ballistics"));Assert.That(model.HasMissingActiveResearch,Is.False);Assert.That(model.Remaining,Is.EqualTo(17));Assert.That(model.CaptureForPersistence().ActiveResearchId,Is.EqualTo("core.research.ballistics"));}
