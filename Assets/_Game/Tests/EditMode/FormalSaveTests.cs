...
namespace WasteCity.Tests
{
    public sealed class FormalSaveTests
    {
        ...
        [Test] public void ElixirRoundTripsInCurrentSchema(){var restored=FormalSaveCodec.Decode(FormalSaveCodec.Encode(new FormalSaveData{elixir=4}));Assert.That(restored.schema,Is.EqualTo(22));Assert.That(restored.elixir,Is.EqualTo(4));}
        [Test] public void VersionTwentyTwoPuppetsRoundTrip(){var restored=FormalSaveCodec.Decode(FormalSaveCodec.Encode(new FormalSaveData{puppetProgress=7.5f,puppets=new[]{new FriendlyUnitSnapshot{x=2f,y=-3f,health=91}}}));Assert.That(restored.schema,Is.EqualTo(22));Assert.That(restored.puppetProgress,Is.EqualTo(7.5f));Assert.That(restored.puppets.Length,Is.EqualTo(1));Assert.That(restored.puppets[0].health,Is.EqualTo(91));}
