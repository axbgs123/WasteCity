...
- expose no new public API for malformed-symbol testing. Instead, keep a small `internal` plain-data source-index seam in `ProjectQualityScanner.cs`, grant only `WasteCity.EditModeTests` friend access with `InternalsVisibleTo`, and have production `Scan(string projectRoot)` route through the same seam. Tests may inject type names, sequence-point document paths, and exact `MonoScript` fallback candidates as strings to prove missing, outside-root, and ambiguity failures without modifying real `Library` DLL/PDB files. The seam must not expose Cecil types, bypass production validation, or change non-current-project fixture scanning;
