...
- `BuildingUnlockEvaluationWorkspace` and `BuildingPlacementEvaluationWorkspace` are public solely because `WasteCity.Graybox3D.Building` must call the overloads across the asmdef boundary. Their mutable buffers, reset operation, and read-only views are private/internal implementation details: callers can construct and pass a workspace but cannot mutate or retain its backing lists. The workspace overloads clear and refill their own buffers synchronously on every call; a result returned from either overload is valid only until the next evaluation using that same workspace. The existing four-argument unlock overload and one-argument placement overload continue to allocate independent immutable read-only snapshots with their current lifetime/semantics. No static mutable workspace is permitted.
- `GrayboxBuildingPlacementController3D` owns exactly one `BuildingPlacementEvaluationWorkspace` for the configured session lifetime. It obtains `request.Unlock` by calling the five-argument unlock overload with `workspace.Unlock`, then calls the two-argument placement overload with the same placement workspace. It never exposes the workspace. `CurrentEvaluation` is the last result from that workspace and remains fully readable until the controller's next successful or failed evaluation resets it; `Configure`, `OnDisable`, and `OnDestroy` discard the controller-owned workspace/result and no other controller may share it.
...
public void ConfigureDeploymentRequest(IGrayboxDeploymentRequest value);
...
`GrayboxInputRouter.ConfigureDeploymentRequest(IGrayboxDeploymentRequest value)` rejects null, replaces only the deployment-request dependency, and is public so the EditMode integration spy can be injected without subclassing the sealed city controller. Its existing concrete scene configuration remains intact and constructs the private nested `CityDeploymentRequestAdapter : IGrayboxDeploymentRequest`; the adapter holds one non-null `GrayboxMobileCityController3D`, exposes `CityMode Mode => city.Mode`, and delegates exactly once to `city.TryToggleDeployment(out failureReason)`. The base router owns this adapter/dependency for its configured lifetime and clears it on teardown; it does not reference the downstream Building assembly.
...
`CatalogRevision` is the frozen public `uint` catalog-projection revision. It advances with unchecked arithmetic and never resets during the lifetime of a session component. It increments exactly once after a fixture/model rebuild, an actual route-contact set change, each newly completed research ID, and each successfully committed building completion (both a normal `TickConstruction` completion and each individual successful `CompleteAllConstructionForDevelopment` completion). The completion increment occurs only after its presentation update has succeeded. No-op calls, partial construction progress, and an unobservable validation/presentation failure that rolls itself back before publishing a catalog-projection change do not advance it. A later failure cleanup that reverses a previously committed, revision-published catalog-projection change is itself an actual change and advances it once. `UnlockRouteForDevelopment` and `UnlockAllResearchForDevelopment` therefore use the same actual-change rules: each route/contact or research ID that was already present is a no-op, while each newly added research ID advances the revision once.
...
- Task 7 owns the future extension of this revision contract: every successfully committed owner, state, evacuation-lock, or evacuation-commit mutation that changes `CompletedBuildingCount` increments `CatalogRevision` once. A validation/presentation failure that rolls back before it has published such a change does not increment; however, `RollbackEvacuationLocksAfterFailure` (or equivalent failure cleanup) that unlocks/restores a previously revision-published evacuation lock or owner/state condition and thereby changes `CompletedBuildingCount` back increments once for that restoration. Task 4 does not add those Task 7 mutation paths.
...
- `RollbackEvacuationLocksAfterFailure` accepts only the controller's current suffix/list of still-locked exact work snapshots and exists solely for confirmation/commit failure cleanup before processing can continue. The controller removes a work item from that rollback buffer only after a successful commit. Releasing any previously revision-published lock that restores `CompletedBuildingCount` advances `CatalogRevision` once per restored count-changing instance; releasing an unobservably failed lock does not. The method is not exposed through UI and does not create a player cancellation path after a manifest has been confirmed.
...
- A mutation/presentation exception rolls back ownership/state or list position, grid occupancy, accepted refund, and the current lock snapshot before rethrowing. If presentation recreation during rollback fails, throw an `InvalidOperationException` containing both failures. The controller catches confirmation/commit failure, calls `RollbackEvacuationLocksAfterFailure` for every remaining queued work item, clears its queue, and returns to the open manifest without leaving a lock. That cleanup advances `CatalogRevision` for each previously published lock/owner/state restoration that changes `CompletedBuildingCount`, so the revision-cached menu cannot remain stale; a normal validation failure never partially mutates or advances it.
...
The concrete `Configure(session, GrayboxMobileCityController3D, presentation, menu)` remains the scene-compatible API. It constructs the private nested `CityDeploymentRequestAdapter : IGrayboxDeploymentRequest3D`, whose constructor takes the non-null sealed city controller, exposes `CityMode Mode => city.Mode`, and whose sole command method delegates once to `city.TryToggleDeployment(out failureReason)`. The public interface overload exists only for EditMode spies; it stores the non-null request dependency and does not create/serialize an adapter. Neither adapter nor request is serialized, and `OnDisable`/`OnDestroy` clear the stored request after the idempotent evacuation cleanup.

`TryHandleDeploymentRequest()` reports **F request consumption**, not city-transition success. It returns true while a manifest/processing state owns F, and after making its one no-ground `IGrayboxDeploymentRequest3D.TryToggleDeployment(out _)` delegation even when that call returns false. Task 9 supplies one spy implementing both public request interfaces with one shared counter, injects it into evacuation and `GrayboxInputRouter.ConfigureDeploymentRequest`, then executes the production `buildingInterceptor.ProcessCurrentInput()` → `baseRouter.ProcessFrame(frame, suppression)` sequence. A failed F must leave the counter at exactly one; a base fallback would invoke the same spy a second time and fail the test.
...
- [ ] Write RED `CatalogRevision` tests for the frozen `public uint CatalogRevision { get; }` contract: a fixture/model rebuild advances it once with unchecked arithmetic; an actual route-contact addition/removal and each newly completed research ID advance it once; repeated/no-op route or research calls do not. Prove one or more successful non-completing `TickConstruction` updates call `presentation.UpdateInstance` yet leave the exact revision unchanged. Prove a normal `TickConstruction` completion advances once only after `presentation.UpdateInstance` succeeds, a presentation failure rollback leaves it unchanged, and `CompleteAllConstructionForDevelopment` advances once per successfully committed instance while already-completed items do not advance it.
...
Remediations A, B, and C are separate remediation commits and separate independent reviews. This remediation is one independent Task 7 commit and one independent review, performed after the original Task 7 commit; the original Task 7 staging checklist is historical baseline only and does not authorize remediation changes. It is limited to the exact remediation allowlist below, not every original Task 7 file. If implementation needs another production path, stop, revise this plan, and obtain approval before editing it. The evacuation result contract produced here is consumed by Task 9's already-approved input-router paths to complete the exact-once F gate; this work does not add an F10 reader. Task 11 alone owns virtual-device panel-click acceptance.

- [ ] Start after the original Task 7 commit with only Task 7-owned paths clean; snapshot the permitted MCP/user dirty state, which is never staged or committed:

```bash
task7_external=/tmp/wastecity-3d-building/task-07-remediation-external-dirty.txt
git status --short -- \
  Packages/manifest.json Packages/packages-lock.json \
  ProjectSettings/PackageManagerSettings.asset ProjectSettings/URPProjectSettings.asset \
  > "$task7_external"
if ! git diff --cached --quiet -- \
  Packages/manifest.json Packages/packages-lock.json \
  ProjectSettings/PackageManagerSettings.asset ProjectSettings/URPProjectSettings.asset; then
  echo "MCP/user external paths must remain unstaged" >&2
  exit 1
fi
if ! git diff --quiet -- \
  Assets/_Game/Scripts/Graybox3D/Building/GrayboxBuildingSession3D.cs \
  ...
  Assets/_Game/Tests/EditMode/GrayboxEvacuationTests.cs \
  Assets/_Game/Tests/EditMode/GrayboxBuildingSessionTests.cs \
  Assets/_Game/Tests/EditMode/GrayboxBuildingUiAndInputTests.cs; then
  echo "Task 7-owned paths must start clean" >&2
  exit 1
fi
git rev-parse HEAD > /tmp/wastecity-3d-building/task-07-remediation-base.txt
```

The remediation-only allowlist is exactly these existing paths, with no `.meta`, pure-rule, scene, Task 10, or Task 11 path: `Assets/_Game/Scripts/Graybox3D/Building/GrayboxBuildingSession3D.cs`, `Assets/_Game/Scripts/Graybox3D/Building/GrayboxConstructionController3D.cs`, `Assets/_Game/Scripts/Graybox3D/Building/GrayboxBuildingMenuView3D.cs`, `Assets/_Game/Scripts/Graybox3D/Building/GrayboxEvacuationController3D.cs`, `Assets/_Game/Tests/EditMode/GrayboxEvacuationTests.cs`, `Assets/_Game/Tests/EditMode/GrayboxBuildingSessionTests.cs`, and `Assets/_Game/Tests/EditMode/GrayboxBuildingUiAndInputTests.cs`.
...
- [ ] Add RED evacuation-result tests with a call-count spy implementing public `IGrayboxDeploymentRequest3D`, including `CityMode Mode { get; }`. The evacuation layer must report that it consumed F independently from whether the delegated `TryToggleDeployment(out _)` succeeded: when it has no player-owned ground building and the spy returns false, its single request is still consumed; manifest-open and processing states consume F without requesting deployment. Task 9 adds the same-counter base-router assertion.
...
```bash
task7_remediation_base=$(cat /tmp/wastecity-3d-building/task-07-remediation-base.txt)
task7_allowlist=/tmp/wastecity-3d-building/task-07-remediation-allowlist.txt
task7_changed=/tmp/wastecity-3d-building/task-07-remediation-changed.txt
printf '%s\n' \
  Assets/_Game/Scripts/Graybox3D/Building/GrayboxBuildingSession3D.cs \
  Assets/_Game/Scripts/Graybox3D/Building/GrayboxConstructionController3D.cs \
  Assets/_Game/Scripts/Graybox3D/Building/GrayboxBuildingMenuView3D.cs \
  ...
  Assets/_Game/Tests/EditMode/GrayboxBuildingSessionTests.cs \
  Assets/_Game/Tests/EditMode/GrayboxBuildingUiAndInputTests.cs \
  | LC_ALL=C sort > "$task7_allowlist"
task7_external_allowlist=/tmp/wastecity-3d-building/task-07-remediation-external-allowlist.txt
task7_all_changed=/tmp/wastecity-3d-building/task-07-remediation-all-changed.txt
printf '%s\n' \
  Packages/manifest.json Packages/packages-lock.json \
  ProjectSettings/PackageManagerSettings.asset ProjectSettings/URPProjectSettings.asset \
  | LC_ALL=C sort > "$task7_external_allowlist"
git diff --name-only "$task7_remediation_base" | LC_ALL=C sort > "$task7_all_changed"
comm -23 "$task7_all_changed" "$task7_external_allowlist" > "$task7_changed"
if comm -23 "$task7_changed" "$task7_allowlist" | grep -q .; then
  echo "Task 7 remediation changed a non-approved path" >&2
  comm -23 "$task7_changed" "$task7_allowlist" >&2
  exit 1
fi
rg -n "TryCancelConstruction|IsEvacuationLocked|RollbackEvacuationLocksAfterFailure|OnDisable|OnDestroy|Configure" \
  ...
  Assets/_Game/Scripts/Graybox3D/Building/GrayboxEvacuationController3D.cs \
  Assets/_Game/Scripts/Graybox3D/Building/GrayboxBuildingMenuView3D.cs
git diff --check "$task7_remediation_base" -- $(cat "$task7_allowlist")
```
...
- [ ] Stage the remediation-only Task 7 commit after the allowlist gate, not the original Task 7 files:
...
git add Assets/_Game/Tests/EditMode/GrayboxBuildingSessionTests.cs
git add Assets/_Game/Tests/EditMode/GrayboxBuildingUiAndInputTests.cs
git diff --cached --name-only | LC_ALL=C sort > /tmp/wastecity-3d-building/task-07-remediation-staged.txt
if comm -23 /tmp/wastecity-3d-building/task-07-remediation-staged.txt "$task7_allowlist" | grep -q .; then
  echo "Task 7 remediation staged a non-approved path" >&2
  exit 1
fi
git commit -m "fix: harden graybox evacuation lifecycle"
```
...
- [ ] Implement pure rules using `ConstructionRefundRules`. At `ConfirmManifest`, create every immutable work snapshot once, sort Full work by ordinal stable ID, and call session `TryLockEvacuationWork` once before any timer or immediate commit. Session prevalidates the whole list, copies exact work values into its private lock table, marks instances, and makes `TickConstruction` skip all locks. Implement the controller's retained concrete `Configure(..., GrayboxMobileCityController3D, ...)` through its private `CityDeploymentRequestAdapter`, and use the public `IGrayboxDeploymentRequest3D` overload only for the exact call-count spy. Both paths call the session's `CopyPlayerOwnedGroundInstances`, `HasPlayerOwnedGroundInstances`, `TryLockEvacuationWork`, `RollbackEvacuationLocksAfterFailure`, and `TryCommitEvacuation(in BuildingEvacuationWork, ...)` APIs. Category batches call only presenter `CategoryOf`; the controller must not copy category mappings or write `CityDeploymentModel.Mode`, Transform, grid private cells or inventory internals.
...
This remediation is one independent Task 8 commit and one independent review, performed after the original Task 8 commit; the original Task 8 staging checklist is historical baseline only and does not authorize remediation changes. Its approved production surface is exactly `GrayboxDeveloperModifierBootstrap3D.cs` and `GrayboxDeveloperModifier3D.cs`; `GrayboxMobileCityController3D` receives no new behavior. Its approved tests are exactly `GrayboxDeveloperModifierTests.cs` and `GrayboxBuildingUiAndInputTests.cs`. If another production path is required, stop and revise the plan before implementation.

- [ ] Start after the original Task 8 commit with only Task 8-owned paths clean; preserve the MCP/user dirty-state allowlist, which is never staged or committed:

```bash
task8_external=/tmp/wastecity-3d-building/task-08-remediation-external-dirty.txt
git status --short -- \
  Packages/manifest.json Packages/packages-lock.json \
  ProjectSettings/PackageManagerSettings.asset ProjectSettings/URPProjectSettings.asset \
  > "$task8_external"
...
if ! git diff --quiet -- \
  ...
  Assets/_Game/Tests/EditMode/GrayboxDeveloperModifierTests.cs \
  Assets/_Game/Tests/EditMode/GrayboxBuildingUiAndInputTests.cs \
  || ! git diff --cached --quiet -- \
  Assets/_Game/Scripts/Graybox3D/Building/GrayboxDeveloperModifierBootstrap3D.cs \
  Assets/_Game/Scripts/Graybox3D/Building/GrayboxDeveloperModifier3D.cs \
  Assets/_Game/Tests/EditMode/GrayboxDeveloperModifierTests.cs \
  Assets/_Game/Tests/EditMode/GrayboxBuildingUiAndInputTests.cs; then
  echo "Task 8-owned paths must start clean" >&2
  exit 1
fi
git rev-parse HEAD > /tmp/wastecity-3d-building/task-08-remediation-base.txt
```

The remediation-only allowlist is exactly `Assets/_Game/Scripts/Graybox3D/Building/GrayboxDeveloperModifierBootstrap3D.cs`, `Assets/_Game/Scripts/Graybox3D/Building/GrayboxDeveloperModifier3D.cs`, `Assets/_Game/Tests/EditMode/GrayboxDeveloperModifierTests.cs`, and `Assets/_Game/Tests/EditMode/GrayboxBuildingUiAndInputTests.cs`. No city-controller, `.meta`, scene, Task 10, or Task 11 file is allowed.
...
- [ ] Add RED edit-time tests with `Application.isPlaying == false` that call `Awake`/`Configure` and prove no savable/generated panel, Canvas, EventSystem, `InputSystemUIInputModule`, or command object is created. In the same approved EditMode fixture, use `[UnitySetUp]` to snapshot `EditorSettings.enterPlayModeOptionsEnabled` and `EditorSettings.enterPlayModeOptions`; the `[UnityTest] IEnumerator` yields `new EnterPlayMode()`, creates the complete runtime-only session/city/presentation/shared-Canvas fixture, calls `Configure`/`TryTogglePanel`, and proves the generated panel is a child of that Canvas with no second Canvas/EventSystem/input module. Its non-yielding `finally` destroys owned runtime fixture objects only. A separate `[UnityTearDown] IEnumerator` always yields `new ExitPlayMode()` when needed, then destroys edit-time fixtures with `DestroyImmediate` and restores both EditorSettings values. No iterator yields from `finally`. The test command remains `-testPlatform EditMode`; it exercises the public Play Mode lifecycle without adding a PlayMode scene test. Task 10 remains the sole owner of the serialized EventSystem/InputSystemUIInputModule.
...
```bash
task8_remediation_base=$(cat /tmp/wastecity-3d-building/task-08-remediation-base.txt)
task8_allowlist=/tmp/wastecity-3d-building/task-08-remediation-allowlist.txt
task8_changed=/tmp/wastecity-3d-building/task-08-remediation-changed.txt
printf '%s\n' \
  Assets/_Game/Scripts/Graybox3D/Building/GrayboxDeveloperModifierBootstrap3D.cs \
  Assets/_Game/Scripts/Graybox3D/Building/GrayboxDeveloperModifier3D.cs \
  Assets/_Game/Tests/EditMode/GrayboxDeveloperModifierTests.cs \
  Assets/_Game/Tests/EditMode/GrayboxBuildingUiAndInputTests.cs \
  | LC_ALL=C sort > "$task8_allowlist"
task8_external_allowlist=/tmp/wastecity-3d-building/task-08-remediation-external-allowlist.txt
task8_all_changed=/tmp/wastecity-3d-building/task-08-remediation-all-changed.txt
printf '%s\n' \
  Packages/manifest.json Packages/packages-lock.json \
  ProjectSettings/PackageManagerSettings.asset ProjectSettings/URPProjectSettings.asset \
  | LC_ALL=C sort > "$task8_external_allowlist"
git diff --name-only "$task8_remediation_base" | LC_ALL=C sort > "$task8_all_changed"
comm -23 "$task8_all_changed" "$task8_external_allowlist" > "$task8_changed"
if comm -23 "$task8_changed" "$task8_allowlist" | grep -q .; then
  echo "Task 8 remediation changed a non-approved path" >&2
  comm -23 "$task8_changed" "$task8_allowlist" >&2
  exit 1
fi
if ! git diff --exit-code "$task8_remediation_base" -- \
  Assets/_Game/Scripts/Graybox3D/GrayboxMobileCityController3D.cs; then
  echo "Task 8 remediation must not change GrayboxMobileCityController3D" >&2
  exit 1
fi
if git diff -U0 "$task8_remediation_base" -- \
  Assets/_Game/Scripts/Graybox3D/GrayboxMobileCityController3D.cs | \
  rg -n '^\+.*(RestoreDeploymentForDevelopment|CompleteDeploymentTransitionForDevelopment|public .*Development)'; then
  echo "Task 8 must not add a city-controller convenience declaration" >&2
  exit 1
fi
if rg --pcre2 -n "new\\s+GameObject\\s*\\([^\\n;]*(?:typeof\\s*\\(\\s*(?:Canvas|EventSystem|InputSystemUIInputModule)\\s*\\)|(?:Canvas|EventSystem|InputSystemUIInputModule))|AddComponent\\s*<\\s*(?:Canvas|EventSystem|InputSystemUIInputModule)\\s*>|AddComponent\\s*\\(\\s*typeof\\s*\\(\\s*(?:Canvas|EventSystem|InputSystemUIInputModule)\\s*\\)\\s*\\)|new\\s+(?:Canvas|EventSystem|InputSystemUIInputModule)\\b" \
  Assets/_Game/Scripts/Graybox3D/Building/GrayboxDeveloperModifierBootstrap3D.cs \
  Assets/_Game/Scripts/Graybox3D/Building/GrayboxDeveloperModifier3D.cs; then
  echo "Task 8 modifier must use the supplied Building Canvas and create no Canvas/EventSystem/input module" >&2
  exit 1
fi
git diff --check "$task8_remediation_base" -- $(cat "$task8_allowlist")
```

Expected: the city-controller diff is empty, no new city convenience declaration exists, permitted calls from the exact modifier methods remain allowed, and no second Canvas/EventSystem/input module is created. No Task 8 staging command may include the city file.

- [ ] Stage the remediation-only Task 8 commit:
...
git add Assets/_Game/Tests/EditMode/GrayboxBuildingUiAndInputTests.cs
git diff --cached --name-only | LC_ALL=C sort > /tmp/wastecity-3d-building/task-08-remediation-staged.txt
if comm -23 /tmp/wastecity-3d-building/task-08-remediation-staged.txt "$task8_allowlist" | grep -q .; then
  echo "Task 8 remediation staged a non-approved path" >&2
  exit 1
fi
git commit -m "fix: harden graybox developer modifier lifecycle"
```
...
- [ ] Under the Editor test compilation, write RED command tests for resource +100/+1000/clear/set with capacity rules; single research/route/all unlock without relocking; `SetCityMode` and `CompleteCityTransition` calling the already-accepted existing city development methods without changing their file; construction multiplier 1×/10×/100×; immediate completion finishing every eligible unlocked site in the same frame through `CompleteAllConstructionForDevelopment`; immediate completion preserving the prior multiplier so the next site is not accelerated; exit/session recreation discarding all changes.

- [ ] Add spy/model-state assertions proving commands only call `ResourceInventory`, `ResearchModel`, session methods, and the accepted existing city development methods. Assert no direct Transform assignment, grid field reflection, city-controller extension, or bypassed placement confirmation.
...
Expected RED: missing bootstrap/modifier behavior only; existing accepted city methods are invoked through the frozen modifier surface without a city-controller diff.
...
- [ ] Implement the modifier only against the pre-existing approved public city/session interfaces. Do not modify `GrayboxMobileCityController3D`, add a city development adapter, set Transform/Collider/renderer state, reflect into city/grid fields, or create a bypass around placement/deployment rules. `SetCityMode` and `CompleteCityTransition` use only the already-accepted existing city development methods; conflicting modifier convenience APIs are removed or private.
...
```bash
task9_external=/tmp/wastecity-3d-building/task-09-remediation-external-dirty.txt
git status --short -- \
  Packages/manifest.json Packages/packages-lock.json \
  ProjectSettings/PackageManagerSettings.asset ProjectSettings/URPProjectSettings.asset \
  > "$task9_external"
...
if ! git diff --quiet -- \
  ...
  echo "Task 9-owned paths must start clean" >&2
  exit 1
fi
...
- [ ] Add RED exact-once F integration tests with one `SharedDeploymentRequestSpy` implementing both `IGrayboxDeploymentRequest3D` and `IGrayboxDeploymentRequest`, exposing `CityMode Mode { get; }`, returning false from `TryToggleDeployment(out string)`, and incrementing one shared counter. Inject that one instance into `GrayboxEvacuationController3D.Configure(..., IGrayboxDeploymentRequest3D, ...)` and `GrayboxInputRouter.ConfigureDeploymentRequest(IGrayboxDeploymentRequest)`, configure the building interceptor on the base router, then execute the production interceptor result into `baseRouter.ProcessFrame(frame, suppression)`. With no ground buildings and a failed F, assert counter `== 1`; a base fallback increments the same spy to `2`. Assert manifest-open and processing consume F with counter `== 0`.
...
```bash
task9_remediation_base=$(cat /tmp/wastecity-3d-building/task-09-remediation-base.txt)
task9_allowlist=/tmp/wastecity-3d-building/task-09-remediation-allowlist.txt
task9_changed=/tmp/wastecity-3d-building/task-09-remediation-changed.txt
printf '%s\n' \
  Assets/_Game/Scripts/Graybox3D/IGrayboxInputInterceptor.cs \
  Assets/_Game/Scripts/Graybox3D/GrayboxInputRouter.cs \
  Assets/_Game/Scripts/Graybox3D/Building/GrayboxBuildingInputRouter3D.cs \
  ...
  Assets/_Game/Tests/EditMode/GrayboxBuildingUiAndInputTests.cs \
  Assets/_Game/Tests/EditMode/GrayboxCameraAndInputTests.cs \
  | LC_ALL=C sort > "$task9_allowlist"
task9_external_allowlist=/tmp/wastecity-3d-building/task-09-remediation-external-allowlist.txt
task9_all_changed=/tmp/wastecity-3d-building/task-09-remediation-all-changed.txt
printf '%s\n' \
  Packages/manifest.json Packages/packages-lock.json \
  ProjectSettings/PackageManagerSettings.asset ProjectSettings/URPProjectSettings.asset \
  | LC_ALL=C sort > "$task9_external_allowlist"
git diff --name-only "$task9_remediation_base" | LC_ALL=C sort > "$task9_all_changed"
comm -23 "$task9_all_changed" "$task9_external_allowlist" > "$task9_changed"
if comm -23 "$task9_changed" "$task9_allowlist" | grep -q .; then
  echo "Task 9 remediation changed a non-approved path" >&2
...
git diff --check "$task9_remediation_base" -- $(cat "$task9_allowlist")
```
...
- [ ] Stage the remediation-only Task 9 commit after the allowlist gate, not the original Task 9 files:
...
git diff --cached --name-only | LC_ALL=C sort > /tmp/wastecity-3d-building/task-09-remediation-staged.txt
if comm -23 /tmp/wastecity-3d-building/task-09-remediation-staged.txt "$task9_allowlist" | grep -q .; then
  echo "Task 9 remediation staged a non-approved path" >&2
  exit 1
fi
git commit -m "fix: harden graybox dynamic evaluation"
```
...
- [ ] Implement `IGrayboxInputInterceptor` and public `IGrayboxDeploymentRequest` in the base assembly, a serialized optional `MonoBehaviour` field in `GrayboxInputRouter`, and public `ConfigureDeploymentRequest(IGrayboxDeploymentRequest)`. Validate the input interceptor in `ConfigureInputInterceptor`; retain existing concrete city setup by wrapping it in the frozen private city adapter. Preserve all existing overloads and runtime tick order.
...
### Task 10 acceptance remediation: reload-safe evacuation and final authoring bytes

The original four Task 10 paths remain approved. This acceptance remediation additionally permits only `Assets/_Game/Scripts/Graybox3D/Building/GrayboxEvacuationController3D.cs`, `Assets/_Game/Tests/EditMode/GrayboxEvacuationTests.cs`, and this plan file. Any requirement for another production, scene, or test path is a stop gate.

- [ ] Add RED lifecycle tests in `GrayboxEvacuationTests.cs` that assign only the serialized `session`, `city`, `presentation`, and `menu` references, never call either public `Configure` overload, and then exercise the real first `OnEnable` lifecycle. With no player-owned ground instance, `TryHandleDeploymentRequest()` must be consumed exactly once and the real city deployment state must advance through its existing API. Disable then re-enable the same component and prove the serialized references restore an equivalent usable deployment path again. These tests must fail only because the non-serialized `deploymentRequest` and `evacuationPresentation` are not reconstructed.
- [ ] Refactor `GrayboxEvacuationController3D` without adding a second deployment rule. Add a private serialized-dependency restoration path that, only when all four serialized references are present, creates `new CityDeploymentRequestAdapter(city)` and assigns `evacuationPresentation = presentation`. `OnEnable` restores these runtime-only dependencies before subscribing to the menu. Split cleanup so `OnDisable` still unsubscribes, rolls back every outstanding lock exactly once, releases the menu block, and clears transient processing/adapters while retaining the four serialized references for a later enable; `OnDestroy` and dependency replacement may clear the serialized references after the same idempotent rollback/unbind work. Both public `Configure` overloads keep their existing injectable behavior and replace old dependencies safely. Existing Configure(old→new), disable, destroy, repeated cleanup, committed-item, rollback, revision, and menu-unbind tests must remain green.
- [ ] Extend `GrayboxSceneContractTests.cs` with RED cases that temporarily break the loaded scene's `GrayboxUrpScope.pipelineAsset` and `GrayboxWorldView3D.sharedMaterial` references in memory, invoke `ValidateFoundationContract`, and require `InvalidOperationException` before any authoring mutation or save. Restore the in-memory values in `finally` without saving. `ValidateFoundationContract` must load the approved assets read-only from `PipelinePath` and `MaterialPath` and require exact serialized reference identity before `Configure` calls any renderer, pipeline, material, scene, or build-settings mutation.
- [ ] Extend the platform contract RED to require the visible cube and selection Collider to cover exactly `2.56 × 1.92` in city-local XZ, use one visible Renderer with the shared graybox material, and place the visible top surface exactly `0.01` above the mobile-city body top. The minimal authoring implementation may scale the single cube proxy and use its Collider as the selection proxy; neither Renderer nor Collider becomes placement truth.
- [ ] Do not edit scene YAML after authoring. If Unity serialization emits trailing spaces, add a deterministic authoring-owned normalization step inside `GrayboxSceneAuthoring.Configure` after `SaveScene`: normalize only trailing spaces/tabs, reimport and reopen the scene through public Editor APIs, rerun `ValidateFoundationContract`, and leave the final on-disk bytes as the bytes tested and hashed. The second complete `Configure` pass must produce an identical final scene SHA. No shell, formatter, test teardown, or manual edit may change `GrayboxPrototype3D.unity` after the final authoring pass.
- [ ] Re-run the Task 10 focused scene and evacuation lifecycle tests, Task 7–9 related tests, full EditMode, full PlayMode, and headless compile. Repeat the complete before/after-1/after-2 foundation identity and scene/rendering GUID comparison, and compare the final post-normalization scene byte hash after both authoring passes. Run `git diff --check` only against those final bytes.
- [ ] The independent Task 10 re-review must report zero Critical and zero Important findings before the implementation commit or push. Stage the original four Task 10 paths plus the two remediation paths exactly; never stage the protected Packages or ProjectSettings files.
