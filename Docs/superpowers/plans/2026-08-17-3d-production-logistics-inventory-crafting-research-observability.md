...
> 日期：2026-08-17<br>
> 状态：已批准需求的可执行 TDD 计划，已执行至“已实现待验证”；完整自动化、项目质量门和四个正式构建已通过，`RecordVerification` 在最终实现提交上记录；真实 Windows 验证和用户试玩仍未完成<br>
> 受控需求：`IDEA-0011`<br>
...
7. 运行时验证：真实输入闭环、长期规则时间、重连/暂停/满仓/枯竭、空闲 UI、非零合成时间，以及跨真实 fixed step 但 revision 不变的生产路径 `0 B`；活跃生产的新不可变快照只允许在固定 `10 Hz` revision 变化时按相关建筑数量线性、有界分配；
