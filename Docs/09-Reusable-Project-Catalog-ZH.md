...
`IDEA-0011` 计划在默认 3D 场景增加背包、建筑缓存详情、真实 `E` 输入和双向拖拽。该界面应只投影领域库存，不持有或直接改写资源真值；拖拽通过可测试的原子转移规则提交，并遵守系统菜单、取消链、界面焦点和世界点击优先级。当前尚无完成的生产类型、资源路径或通过验证的测试，因此本条不是“推荐复用”入口，也不得登记不存在的代码名；实现和质量目录验证完成后再按真实所有者决定“复用前审查”或“仅限场景”。
...
### 三维生产配置、建筑缓存与玩家背包（计划项，待实现）

`IDEA-0011` 计划新增由稳定建筑 ID 和资源 ID 驱动的生产配置、逐建筑独立周期与内部缓存、城市物流可访问库存、玩家背包和原子转移规则。已有 `ResourceInventory`、`WorldMapModel`、`BuildingCatalog`、`BuildingRangeRules` 与 `CityOperationalRules` 是必须消费的现有真值，计划项不能复制资源数量、节点余量、建筑兼容、物流范围或城市倍率。当前设计和测试正在建立，尚无可登记为已实现的类型或文件；完成后应依据真实职责分别评估“生产配置”“建筑生产运行态”“玩家背包模型”和“库存转移规则”的复用等级，并把实际测试写入机器质量目录。
...
`IDEA-0011` 计划为铁矿、合金、弹药、石材、生物质、能晶和水提供缓存/背包物品图标，为现有世界节点资源提供可替换占位表现，并提供统一缺图回退。生成素材必须记录工具、提示词、日期、后期处理和 Unity 最终路径；图标与世界模型都不得持有资源数量、节点类型或生产状态。当前素材尚未生成、接入或通过用户视觉确认，本条不是可复用资产声明；只有资产记录、稳定映射、导入检查和运行时回退验证齐全后，才可把真实目录登记为“复用前审查”。
