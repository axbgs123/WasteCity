...
### 正式三态迷雾、情报与自动扫描运行时（推荐复用）

能解决什么：以现役 `WorldMapModel` 网格为唯一空间基础，集中提供 Hidden/Explored/Visible 三态、城市/领袖/前哨/无人机视野并集、最后情报时效和安全矿区/结晶裂谷首次可见自动扫描。在哪里：`Assets/_Game/Scripts/World/Exploration/ExplorationCatalog.cs`、`Assets/_Game/Scripts/World/Exploration/WorldVisionSource.cs`、`Assets/_Game/Scripts/World/Exploration/WorldVisibilityRuntime.cs`、`Assets/_Game/Scripts/World/Exploration/WorldIntelRuntime.cs`、`Assets/_Game/Scripts/World/Exploration/WorldExplorationRuntime.cs`、`Assets/_Game/Scripts/World/Exploration/ScanRuntime.cs`。怎么复用：调用者只发布带稳定 ID、权威坐标和资格的视野来源；查询三态和只读情报快照，首次可见扫描委托既有 Attention 稳定事件。不能负责什么：不复制地形、Traversal、资源余量、聚落或角色位置；当前 Visible 可重建且不入档，扫描不增加、移动或重置矿点，也不更改地图 seed 与正式内容目录。改后跑哪组测试：`IDEA0029ExplorationCatalogTests`、`IDEA0029WorldVisibilityRuntimeTests`、`IDEA0029WorldIntelRuntimeTests`、`IDEA0029WorldExplorationRuntimeTests`、`IDEA0029ScanRuntimeTests`。代码名：`FormalExplorationCatalog3D`、`WorldVisionSource`、`WorldVisibilityRuntime`、`WorldIntelRuntime`、`WorldExplorationRuntime`、`WorldScanRuntime`。
...
### 领袖控制、基础 AI 与手采运行时（推荐复用）

能解决什么：集中保存 AI/Manual 控制意图，以纯规则解析跟随、回泊和保持，并按正式 `1.5` 格、`6` 秒 `1` 单位规则协调低效率应急手采。在哪里：`Assets/_Game/Scripts/Leader/Exploration/LeaderInteractionCatalog.cs`、`Assets/_Game/Scripts/Leader/Exploration/LeaderControlRuntime.cs`、`Assets/_Game/Scripts/Leader/Exploration/LeaderAiRules.cs`、`Assets/_Game/Scripts/Leader/Exploration/LeaderIntent.cs`、`Assets/_Game/Scripts/Leader/Exploration/ManualGatherRuntime.cs`。怎么复用：先从权威角色事实评估控制资格，再由既有直接控制协调器解析实际目标；手采每周期先预检节点、视野、距离与背包空间，再提交同一矿点和背包。不能负责什么：不建立第二直接控制目标、第二资源余额或第二背包；AI 不自动寻矿、战斗或施法，UI 不得绕过运行时直接写模式或资源。改后跑哪组测试：`IDEA0029LeaderInteractionCatalogTests`、`IDEA0029LeaderControlRuntimeTests`、`IDEA0029LeaderAiRulesTests`、`IDEA0029ManualGatherRuntimeTests`、`GrayboxLeaderControlTests`。代码名：`LeaderInteractionCatalog`、`LeaderControlRuntime`、`LeaderAiRules`、`ManualGatherRuntime`。

### 岑烬求救与原子招募运行时（复用前审查）

能解决什么：拥有求救点发现、`10` 生物质预留、`12` 秒救援、及时/延迟结果和人口 `+40`、关注度 `+5`、角色招募的一次性提交状态。在哪里：`Assets/_Game/Scripts/Leader/Exploration/CenJinDistressRuntime.cs`、`Assets/_Game/Scripts/Leader/Exploration/LeaderInteractionCatalog.cs`。怎么复用：通过显式事务接口协调资源、人口、角色和 Attention owner；只读快照用于场景显示。不能负责什么：取消和失败必须回滚，UI 与读档不得重复结算；既有 schema `36` 岑烬迁为兼容已救援，不补扣资源或补发奖励。改后跑哪组测试：`IDEA0029CenJinRescueRuntimeTests`、`IDEA0029CenJinDistressPresentationTests`。代码名：`CenJinDistressRuntime`、`CenJinDistressSnapshot`、`CenJinDistressTickResult`。
...
能解决什么：在正式 Attention、Fate、Civilization、AdvancementSequence、九命轨效果 owner 与 schema 36 progression/effect DTO 之间确定性捕获、预检并原子恢复 Lv.1/Lv.2 当前真值。在哪里：`Assets/_Game/Scripts/Graybox3D/Building/GrayboxFormalProgressionSaveAdapter3D.cs`。怎么复用：显式注入 Attention、Fate、文明、演出和各命轨 owner，先建立零写入恢复计划，再由存档协调器统一提交或回滚；量子同步键、空间模板、局部时加周期/预算、预知显示、因果历史、宝箱幂等账本和坐标锁定都保留独立所有权。不能负责什么：只映射已经验证的 Progression、文明升阶、演出阶段与命轨效果快照；不计算升阶资格、不提交关注度或奖励、不推进演出、不读写文件或操作 UI。改后跑哪组测试：`GrayboxFormalProgressionSaveAdapterTests`、`GrayboxFormalFateEffectsSaveAdapterTests`、`FormalSaveSchema33ContractTests`、`FormalSaveSchema33MigrationTests`、`FormalSaveSchema36ContractTests`、`GrayboxFormalSaveRuntimeHostTests`、`GrayboxCivilizationAdvancementRuntimeInputTests`。代码名：`GrayboxFormalProgressionRestorePlan3D`、`GrayboxFormalProgressionSaveAdapter3D`。
...
能解决什么：在 `GrayboxPrototype3D` 投影三态迷雾遮罩、资源标记显隐、自动扫描反馈、`L` 领袖面板、手采/救援操作以及前哨状态和警报定位。在哪里：`Assets/_Game/Scripts/Graybox3D/Exploration/GrayboxExplorationController3D.cs`、`Assets/_Game/Scripts/Graybox3D/Building/GrayboxExplorationView3D.cs`、`Assets/_Game/Scripts/Graybox3D/Exploration/GrayboxFogPresenter3D.cs`、`Assets/_Game/Scripts/Graybox3D/Exploration/GrayboxFogVisualPolicy3D.cs`、`Assets/_Game/Scripts/Graybox3D/Exploration/ResourceMarkerFogPresentation3D.cs`、`Assets/_Game/Scripts/Graybox3D/Exploration/GrayboxCenJinDistressPresenter3D.cs`。怎么复用：只消费正式不可变快照，所有按钮与世界选择只发布命令，并服从既有 Input System 模态优先级；自动扫描反馈不得抢占移动。不能负责什么：不保存探索、库存、角色、聚落或警报真值，不从中文文案反推规则；直接调用内部方法不能代替真实输入验收。改后跑哪组测试：`IDEA0029ExplorationController3DTests`、`IDEA0029ExplorationUiProjectionTests`、`IDEA0029FogPresentationTests`、`IDEA0029CenJinDistressPresentationTests`、`GrayboxProductionObservabilityRuntimeInputTests`。代码名：`GrayboxExplorationController3D`、`GrayboxExplorationView3D`、`GrayboxFogPresenter3D`、`GrayboxFogVisualPolicy3D`、`GrayboxCenJinDistressPresenter3D`。
...
能解决什么：从正式 Attention/Fate 不可变快照投影关注度数值、四阶段、下一阈值、最近三条中文原因和本局固定三项命轨候选，并用真实 UGUI 状态按钮打开或关闭详情。在哪里：`Assets/_Game/Scripts/Graybox3D/Building/GrayboxProgressionHudView3D.cs`、`Assets/_Game/Scripts/Graybox3D/Building/GrayboxProgressionHudController3D.cs`。怎么复用：只显示存档或新进度抽取器已经提交的三项及其顺序，不把九条正式池同时塞入三卡区域。不能负责什么：只读取不可变快照并更新真实 UGUI；不写 Attention/Fate runtime、不进入 schema、不复制关注度增量或命轨规则，也不得在 EffectsReady=false 时开放或强制弹出命轨选择。相同快照引用必须保持零重复刷新。改后跑哪组测试：`GrayboxProgressionPresentationTests`、`GrayboxProgressionRuntimeInputTests`。代码名：`GrayboxProgressionHudView3D`、`GrayboxProgressionHudController3D`。

### 三维本局三命轨强制选择 UI（复用前审查）

能解决什么：以真实 UGUI 全屏阻断世界输入，只显示本局从九条正式池稳定抽出的三项候选，并通过二次确认和 ProgressionEventRouter 原子提交唯一 Lv.1 选择与 +5 关注度。在哪里：`Assets/_Game/Scripts/Graybox3D/Building/GrayboxFateSelectionView3D.cs`、`Assets/_Game/Scripts/Graybox3D/Building/GrayboxFateSelectionController3D.cs`。怎么复用：卡片内容、顺序和 stable ID 全部读取 `FormalFateSnapshot.OfferedIds`，继续和回溯时不得重新抽取。不能负责什么：强制选择与 Host/真实输入已接且只负责首次 Lv.1 选择；不直接写 Fate 或 Attention runtime，不提供回溯锚点创建/读取按钮，也不承担文明升阶。Lv.2 由独立文明升阶 UI 与 Host 事务处理。改后跑哪组测试：`GrayboxFateSelectionPresentationTests`、`GrayboxFateSelectionRuntimeInputTests`。代码名：`GrayboxFateSelectionCard3D`、`GrayboxFateSelectionView3D`、`GrayboxFateSelectionController3D`。

### 三维命轨专属详情与操作面板（复用前审查）

能解决什么：保留四份不可变快照和引用缓存显示既有袖珍宇宙、虚空债与回溯锚点状态，同时通过目录驱动状态提供者显示量子纠缠、空间模板、局部时加、预知迟滞、因果透明和虚空宝箱的当前状态与可用动作。在哪里：`Assets/_Game/Scripts/Graybox3D/Building/GrayboxFateOperationsView3D.cs`、`Assets/_Game/Scripts/Graybox3D/Building/GrayboxFateOperationsController3D.cs`。怎么复用：View 只展示已选命轨对应的状态和操作，控制器把通用动作交给 Host 的唯一领域 owner；回溯锚点 Create、Read、Clear 与指定槽读取仍使用二次确认。不能负责什么：不直接写 runtime、schema 或文件；Create/Read/Clear 必须由 Host 绑定到唯一 Rewind Service。未选命轨不开放，读取必须二次确认。改后跑哪组测试：`GrayboxFateOperationsPresentationTests`、`GrayboxFateOperationsRuntimeInputTests`。代码名：`GrayboxFateOperationsView3D`、`GrayboxFateOperationsController3D`。
...
能解决什么：定义袖珍宇宙、量子纠缠、空间模板、回溯锚点、局部时加、预知迟滞、虚空债、因果透明、虚空宝箱九条正式命轨的稳定身份、顺序、显示键、规则键和配置数值。在哪里：`Assets/_Game/Scripts/Progression/FormalFateCatalog.cs`。怎么复用：所有 UI、修改器、抽取器和领域 owner 都按稳定 ID 查询这里，不从中文名字、旧 Legacy 目录或数组位置反推规则。不能负责什么：只定义九条已定稿命轨的静态内容；其余十五条草案不进入正式池，不保存本局抽取、选择或等级，也不执行领域效果。改后跑哪组测试：`FormalFateCatalogTests`。代码名：`FormalFateDefinition`、`FormalFateCatalog`。

### 正式命轨选择运行时（推荐复用）

能解决什么：维护本局已保存的三候选顺序、抽取版本、待选择与已选择状态、命轨等级、Lv.1→Lv.2 单次晋级、revision、缓存快照和原子恢复；旧 schema 35 固定三项保持原顺序。在哪里：`Assets/_Game/Scripts/Progression/FormalFateRuntime.cs`、`Assets/_Game/Scripts/Progression/FormalFateLevelTwoCatalog.cs`。怎么复用：新进度先用 `FormalFateOfferSelector` 生成三项，再把结果交给 runtime；继续、迁移、锚点和波前重试始终恢复保存的候选。不能负责什么：只拥有本局候选、选择与等级真值及 Lv.2 参数；不自行抽取候选，不执行任何命轨领域效果，不增加关注度，不创建检查点，不读写文件、不访问 Unity 或 UI。改后跑哪组测试：`FormalFateRuntimeTests`、`FormalProgressionTests`、`FormalFateLevelTwoCatalogTests`、`FormalFatePerformanceTests`。代码名：`FormalFateSnapshot`、`FormalFateRuntime`。

### 九命轨本局稳定抽取器（推荐复用）

能解决什么：以世界 seed、稳定 session ID 与抽取版本对九条正式命轨执行确定性无放回抽三条，相同输入得到相同候选和顺序。在哪里：`Assets/_Game/Scripts/Progression/FormalFateOfferSelector.cs`。怎么复用：只在创建新进度时调用，并把结果立即交给 `FormalFateRuntime` 和 schema 36 保存。不能负责什么：继续、迁移、回溯锚点和波前重试不得调用它重抽；它不保存状态、不选择命轨也不执行效果。改后跑哪组测试：`FormalFateOfferSelectorTests`。代码名：`FormalFateOfferSelector`。

### 六条新增命轨与坐标锁定纯运行时（推荐复用）

能解决什么：分别持有量子路由状态、单槽空间模板、每规则周期局部时加预算、权威压力预知、完整关注度因果投影、稳定敌人死亡宝箱账本和一次性坐标锁定状态。在哪里：`Assets/_Game/Scripts/Progression/QuantumEntanglementRuntime.cs`、`Assets/_Game/Scripts/Progression/SpatialTemplateRuntime.cs`、`Assets/_Game/Scripts/Progression/LocalHasteRuntime.cs`、`Assets/_Game/Scripts/Progression/ForesightDelayRuntime.cs`、`Assets/_Game/Scripts/Progression/CausalTransparencyRuntime.cs`、`Assets/_Game/Scripts/Progression/VoidChestRuntime.cs`、`Assets/_Game/Scripts/Progression/CoordinateLockRuntime.cs`。怎么复用：每个 owner 只通过自己的快照、预检、提交和恢复边界接入 Host，再由对应领域控制器消费。不能负责什么：不扫描 Unity 场景、不直接写库存/建筑/生产/研究/防御、不把 UI 当真值，也不使用全局 `Time.timeScale`。改后跑哪组测试：`QuantumEntanglementRuntimeTests`、`SpatialTemplateRuntimeTests`、`LocalHasteRuntimeTests`、`ForesightDelayRuntimeTests`、`CausalTransparencyRuntimeTests`、`VoidChestRuntimeTests`、`CoordinateLockRuntimeTests`。代码名：`QuantumEntanglementRuntime`、`SpatialTemplateRuntime`、`LocalHasteRuntime`、`ForesightDelayRuntime`、`CausalTransparencyRuntime`、`VoidChestRuntime`、`CoordinateLockRuntime`。

### 三维命轨领域消费控制器（复用前审查）

能解决什么：把局部时加、空间模板与虚空宝箱命令接到既有 Production/Research/Defense、统一建造施工和 `CityResourceStorageModel` 权威接口。在哪里：`Assets/_Game/Scripts/Graybox3D/Building/GrayboxLocalHasteController3D.cs`、`Assets/_Game/Scripts/Graybox3D/Building/GrayboxSpatialTemplateController3D.cs`、`Assets/_Game/Scripts/Graybox3D/Building/GrayboxVoidChestController3D.cs`。怎么复用：由唯一 Host 注入领域 owner 和现有正式接口；控制器只编排预检、提交与失败回滚。不能负责什么：不得复制目标资格、放置合法性、建造费用、库存容量、死亡身份或掉落幂等判断。改后跑哪组测试：`GrayboxLocalHasteControllerTests`、`GrayboxSpatialTemplateControllerTests`、`GrayboxVoidChestControllerTests`。代码名：`GrayboxLocalHasteController3D`、`GrayboxSpatialTemplateController3D`、`GrayboxVoidChestController3D`。
...
能解决什么：以统一信封区分 legacy 2D schema `1–30`、正式 3D schema `31–37` 和未来版本，提供确定性编码、逐版本历史 payload hash、结构与跨引用验证，以及 `31→32→33→34→35→36→37` 单向迁移。在哪里：`Assets/_Game/Scripts/Persistence/FormalSaveEnvelope.cs`、`Assets/_Game/Scripts/Persistence/FormalSaveCodec.cs`、`Assets/_Game/Scripts/Persistence/FormalSaveValidator.cs`、`Assets/_Game/Scripts/Persistence/ThreeD/FormalThreeDSaveData.cs`、`Assets/_Game/Scripts/Persistence/ThreeD/FormalThreeDExplorationSaveData.cs`。怎么复用：为每个新 schema 保留旧版本精确投影与哈希，再增加单向迁移；schema `37` 保存永久探索、扫描幂等、最后情报、领袖控制/手采、岑烬求救与前哨警报。不能负责什么：只定义存档身份、DTO 信封、codec、纯验证与已批准迁移，不执行文件 IO、领域捕获、恢复应用、实时视野重建或 UI；legacy 2D 不升级为正式 3D，`36→37` 保留既有岑烬与聚落事实，不补扣救援资源、不补发奖励、不猜测历史警报。改后跑哪组测试：`FormalSaveEnvelopeTests`、`FormalSaveValidatorTests`、`FormalSaveSchema32ContractTests`、`FormalSaveDestroyedRuinSchema32Tests`、`FormalSaveSchema33ContractTests`、`FormalSaveSchema33MigrationTests`、`FormalSaveSchema34ContractTests`、`FormalSaveSchema35ContractTests`、`FormalSaveSchema36ContractTests`、`FormalSaveSchema37ContractTests`、`IDEA0027ResearchEffectStateSaveAdapterTests`、`IDEA0029ExplorationSaveAdapterTests`、`GrayboxFormalDefenseCampaignSaveAdapterTests`。代码名：`FormalSaveCheckpointMetadata`、`FormalSaveEnvelope`、`FormalSaveDecodeResult`、`FormalSaveCodec`、`FormalSaveValidationResult`、`FormalSaveValidator`、`FormalThreeDProgressionSaveData`、`FormalThreeDAttentionSaveData`、`FormalThreeDFateSaveData`、`FormalThreeDCivilizationSaveData`、`FormalThreeDWorldSaveData`、`FormalThreeDBuildingsSaveData`、`FormalThreeDStorageSaveData`、`FormalThreeDWarehouseSaveData`、`FormalThreeDBackpackSaveData`、`FormalThreeDCraftingSaveData`、`FormalThreeDCraftingExecutionSaveData`、`FormalThreeDResearchSaveData`、`FormalThreeDResearchEffectStateSaveData`、`FormalThreeDProductionSaveData`、`FormalThreeDProductionStateSaveData`、`FormalThreeDDefenseSaveData`、`FormalThreeDDefenseCampaignSaveData`、`FormalThreeDDefenseCampaignStatisticsSaveData`、`FormalThreeDEvacuationSaveData`、`FormalThreeDEvacuationRuntimePayloadSaveData`、`FormalThreeDExplorationSaveData`。
...
### schema 37 探索、领袖与前哨存档适配器（复用前审查）

能解决什么：在 schema `37` exploration 聚合与永久探索、扫描键、最后情报、领袖控制/手采、岑烬求救和前哨警报 owner 之间确定性捕获、预检、提交与回滚。在哪里：`Assets/_Game/Scripts/Persistence/ThreeD/FormalThreeDExplorationSaveData.cs`、`Assets/_Game/Scripts/Graybox3D/Building/GrayboxExplorationLeaderOutpostSaveAdapter3D.cs`、`Assets/_Game/Scripts/Graybox3D/Building/GrayboxExplorationSaveDomainProxy3D.cs`。怎么复用：作为正式协调器的显式 exploration 领域使用，先验证完整 DTO 与跨引用，再一次提交或完整回滚；成功后从权威来源重建实时视野和表现。不能负责什么：不保存当前 Visible、视野来源、临时遮罩或 UI；`36→37` 必须先验证旧哈希，再从权威坐标建立清洁可视初态且不触发扫描、救援奖励或历史警报。改后跑哪组测试：`FormalSaveSchema37ContractTests`、`IDEA0029ExplorationSaveAdapterTests`、`GrayboxFormalSaveCoordinatorTests`、`GrayboxFormalSaveRuntimeHostTests`。代码名：`FormalThreeDExplorationSaveData`、`GrayboxExplorationLeaderOutpostSaveAdapter3D`、`GrayboxExplorationSaveDomainProxy3D`。

### 正式三维存档领域协调器（复用前审查）

能解决什么：按固定依赖顺序协调 schema `37` 的十一领域捕获与恢复；先完整验证，再事务式应用，失败时回滚全部权威领域并在成功提交后统一重建派生状态。在哪里：`Assets/_Game/Scripts/Graybox3D/Building/GrayboxFormalSaveCoordinator3D.cs`。怎么复用：把探索、领袖交互和前哨警报作为显式领域接入，九命轨细分 owner 继续收敛在 progression 域，不改变其他既有领域所有权。不能负责什么：只协调显式注入的领域和派生重建，不搜索场景、不执行文件 IO、不复制领域规则，也不把实时视野、连接、路径、临时表现或 UI 入档；探索恢复失败与其他领域一样必须完整回滚。改后跑哪组测试：`GrayboxFormalSaveCoordinatorTests`、`GrayboxFormalSaveCheckpointTests`、`GrayboxFormalProgressionSaveAdapterTests`、`GrayboxBuildAndPerformanceTests`、`FormalSaveSchema34ContractTests`、`FormalSaveSchema35ContractTests`、`FormalSaveSchema36ContractTests`、`FormalSaveSchema37ContractTests`、`IDEA0029ExplorationSaveAdapterTests`。代码名：`GrayboxFormalControllerRebuilder3D`、`GrayboxFormalPauseSaveDomain3D`、`GrayboxFormalSaveCoordinatorResult3D`、`GrayboxFormalSaveCoordinator3D`。

### 正式三维存档运行时主机（仅限场景）

能解决什么：在 `GrayboxPrototype3D` 组合 schema `37` 十一领域 coordinator、Attention/Fate/Civilization、探索/领袖/前哨、九命轨效果与文明扩展/高级科技状态唯一 owner、规则效率、UGUI/真实输入、检查点与保存恢复。在哪里：`Assets/_Game/Scripts/Graybox3D/Building/GrayboxFormalSaveRuntimeHost3D.cs`。怎么复用：Host 只负责组合新进度抽取、领域 owner、真实消费者、规则周期、探索/领袖/前哨界面命令和存档边界；继续时先重绑 session 专属 owner 再恢复。不能负责什么：只作为正式 3D 场景组合根；资格从探索、角色、聚落、库存、Attention、Research/Building/Production/Defense 权威快照采集，UI 只发命令。它不用于冻结 2D、不拥有 DTO、迷雾或战斗规则，也不把实时视野、临时表现和输入焦点写入存档。改后跑哪组测试：`GrayboxFormalSaveRuntimeHostTests`、`GrayboxFormalProgressionSaveAdapterTests`、`GrayboxCivilizationAdvancementControllerTests`、`GrayboxCivilizationAdvancementPresentationTests`、`GrayboxCivilizationAdvancementInputCoordinatorTests`、`IDEA0029ExplorationController3DTests`、`IDEA0029ExplorationSaveAdapterTests`、`FormalSaveWaveRetryStoreTests`、`GrayboxFormalSaveRuntimeInputTests`、`GrayboxFormalSaveRoundTripTests`、`GrayboxCivilizationAdvancementRuntimeInputTests`、`GrayboxCivilizationExpansionRuntimeInputTests`。代码名：`GrayboxFormalSaveRuntimeHost3D`。
...
能解决什么：提供主城引用、一次城、一前哨、独立库存/自治、查看与控制权、实体运输队，以及量子纠缠启用时在通信聚落间共享基础资源的库存端点路由。在哪里：`Assets/_Game/Scripts/World/CivilizationExpansion/SettlementRuntime.cs`、`Assets/_Game/Scripts/World/CivilizationExpansion/TransportRuntime.cs`、`Assets/_Game/Scripts/World/CivilizationExpansion/WorldLayerCatalog.cs`、`Assets/_Game/Scripts/World/CivilizationExpansion/QuantumEntanglementInventoryNetwork.cs`。怎么复用：为各 settlement 保留原本地端点，量子网络只在命轨已选且通信有效时把基础资源读写路由到连接成员。不能负责什么：主城仍引用既有库存，位置和路径只消费 `WorldMapModel` 与 `CityPathfinder`；量子网络不创建第二库存、第二地图或第二套主城建筑网格，混合事务失败必须完整回滚。改后跑哪组测试：`IDEA0022WorldLayerSettlementTransportTests`、`QuantumEntanglementRuntimeTests`、`QuantumEntanglementInventoryNetworkTests`。代码名：`WorldLayerRuntime`、`SettlementRuntime`、`TransportRuntime`、`QuantumEntanglementInventoryNetwork`。

### 前哨运行状态与三级警报运行时（推荐复用）

能解决什么：从既有 `SettlementRuntime` 的通信、补给、维护事实派生正常/受限/失联，并保存由权威战斗事实驱动、可确认、可解决和稳定排序的警戒/受袭/危急警报。在哪里：`Assets/_Game/Scripts/World/Exploration/OutpostOperationalState.cs`、`Assets/_Game/Scripts/World/Exploration/OutpostAlertCatalog.cs`、`Assets/_Game/Scripts/World/Exploration/OutpostAlertRuntime.cs`。怎么复用：把 settlement 三项连接状态作为输入，把正式威胁/攻击 owner 的稳定事实提交给警报运行时，UI 只读取快照并发布确认、定位命令。不能负责什么：不创建第二聚落库存、敌人或战斗模拟；断联不等于停产，警报不能靠距离轮询或 UI 伪造，点击只定位并打开详情，不切换直接控制或强制暂停。改后跑哪组测试：`IDEA0029OutpostStateCatalogTests`、`IDEA0029OutpostRuntimeTests`、`IDEA0029OutpostAlertRuntimeTests`。代码名：`OutpostOperationalState`、`OutpostAlertCatalog`、`OutpostAlertRuntime`。
