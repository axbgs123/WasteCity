...
能解决什么：组合统一 ResearchModel 与正式 44 节点 ResearchCatalog，包含已开放的 core.research.legacy-analysis 与可自然研究的剑阵初解，并通过同一文明可用性投影在 Lv.2 自然开放合金装甲/御剑术；同时提交稳定 ID 研究启动、倍率推进、暂停、80% 原子退款与正式目录恢复。在哪里：`Assets/_Game/Scripts/Research/ResearchModel.cs`、`Assets/_Game/Scripts/Research/FormalResearchRuntime.cs`。怎么复用：组合统一 ResearchModel 与正式 44 节点 ResearchCatalog，包含 core.research.legacy-analysis；运行时只管理研究生命周期，科技效果交给独立 ResearchEffectCatalog 从完成集合派生。不能负责什么：不持有研究站、城市模式、暂停或库存真值，不处理 Unity 输入、科技树布局、文件 IO、关注度或效果表现；文明投影只允许两个白名单 ID，不得顺便开放其余 PreviewOnly。改后跑哪组测试：`FormalResearchCatalogTests`、`FormalResearchRuntimeTests`、`ResearchEffectCatalogTests`、`GrayboxFormalResearchSaveAdapterTests`、`GrayboxProductionObservabilityRuntimeInputTests`。代码名：`ResearchCatalog`、`CivilizationResearchAvailability`、`FormalResearchRuntime`。

### 正式科技复合效果目录（推荐复用）

能解决什么：按 44 个正式科技稳定 ID 提供类型化复合效果、前后数值、作用范围、叠加规则与 Executable/Preview 接线状态，并从已完成科技集合确定性派生生产、研究、物流、耐久、预警、再生和击杀回收快照。在哪里：`Assets/_Game/Scripts/Research/ResearchEffectCatalog.cs`。怎么复用：按 44 个正式科技稳定 ID 提供类型化复合效果、前后数值、作用范围、叠加规则与 Executable/Preview 接线状态，并从已完成科技集合确定性派生生产、研究、物流、耐久、预警、再生和击杀回收快照。不能负责什么：不持有完成科技真值、不保存 applied 标志、不处理 UI 或输入；临时特质、护盾和跨帧减益保持 Preview。整数吞吐增益通过周期缩短表达，schema 保持 34。改后跑哪组测试：`ResearchEffectCatalogTests`、`ResearchProductionEffectIntegrationTests`、`ResearchEffectProductionRuntimeTests`、`ResearchRuntimeEffectIntegrationTests`。代码名：`ResearchEffectDefinition`、`ResearchEffectCatalog`、`ResearchEffectSnapshot`、`ResearchEffectResolver`、`ResearchKillRewardResolver`、`FormalProductionResearchModifierAdapter`。
