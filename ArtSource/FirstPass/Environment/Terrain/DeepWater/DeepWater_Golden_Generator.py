...
def make_ora(np, Image, source_root: Path, base_rgb, masks):
    ...
    for index, name in enumerate(("water", "ripple", "oil", "rust")):
    ...
    ora_path = source_root / "DeepWater_Golden_Master.ora"
    stack_layers = []
    ...
    stack_xml = (
        ...
        f'<image version="0.0.1" w="{size}" h="{size}" name="DeepWater Golden Master">\n'
        '  <stack name="DeepWater Golden Source">\n    '
        + "\n    ".join(stack_layers)
        + "\n  </stack>\n</image>\n"
    )
...
def generate_maps(args: argparse.Namespace):
    ...
    if args.concept is None or not args.concept.is_file():
        raise FileNotFoundError("--concept must point to the user-approved DeepWater concept image")

    with Image.open(args.concept) as concept_image:
        concept_image = concept_image.convert("RGB")
        # The approved concept deliberately includes a Wasteland shoreline for art-direction
        # validation. The production tile uses only its dark-water region; shoreline remains
        # an independent later transition asset so the terrain texture carries no gameplay truth.
        water_right = max(1, round(concept_image.width * 0.62))
        concept_image = concept_image.crop((0, 0, water_right, concept_image.height))
    ...
    target_luminance = np.clip(
        0.115 + (luminance - broad_light) * 0.28 + (broad_light - broad_light.mean()) * 0.025,
        0.035,
        0.24,
    )
    ...
    blur_small = gaussian(np, luminance, max(0.8, size / 1024.0 * 2.0))
    blur_medium = gaussian(np, luminance, max(2.0, size / 1024.0 * 14.0))
    high_band = luminance - blur_small
    texture_energy = gaussian(np, np.abs(high_band), max(1.0, size / 1024.0 * 5.0))
    macro = fractal_noise(np, size, [(7, 0.46), (13, 0.32), (23, 0.22)], SEED + 1)
    meso = fractal_noise(np, size, [(23, 0.44), (43, 0.34), (79, 0.22)], SEED + 101)
    fine = fractal_noise(np, size, [(59, 0.42), (113, 0.34), (197, 0.24)], SEED + 201)

    axis = np.arange(size, dtype=np.float32) / size
    yy, xx = np.meshgrid(axis, axis, indexing="ij")
    warp = (macro - 0.5) * 0.65
    wave_a = np.sin(2.0 * np.pi * (3.0 * xx + yy + warp))
    wave_b = np.sin(2.0 * np.pi * (xx - 4.0 * yy - warp * 0.55) + 0.8)
    wave_c = np.sin(2.0 * np.pi * (5.0 * xx + 2.0 * yy) + meso * 0.45)
    wave_d = np.sin(2.0 * np.pi * (9.0 * xx - 3.0 * yy + warp * 0.35) + 1.7)
    wave_field = (wave_a * 0.38 + wave_b * 0.27 + wave_c * 0.20 + wave_d * 0.15).astype(np.float32)
    wave_field = gaussian(np, wave_field, max(0.7, size / 2048.0 * 2.5))

    total = size * size
    available = np.ones((size, size), dtype=bool)
    categories = {}
    raw_scores = {
        "ripple": wave_field * 1.50 + texture_energy * 0.35 + meso * 0.12,
        "oil": gaussian(np, macro * 0.75 + meso * 0.25, max(6.0, size / 2048.0 * 32.0)),
        "rust": gaussian(np, meso * 0.70 + np.maximum(wave_field, 0.0) * 0.30, max(3.0, size / 2048.0 * 10.0)),
    }
    scores = {
        "ripple": gaussian(np, raw_scores["ripple"], max(2.0, size / 2048.0 * 7.0)),
        "oil": raw_scores["oil"],
        "rust": raw_scores["rust"],
    }
    for name in ("oil", "rust", "ripple"):
        categories[name] = pick_top(np, scores[name], available, round(total * RATIOS[name]))
        available &= ~categories[name]
    categories["water"] = available

    radius_scale = size / 2048.0
    blur_radius = {"water": 10.0, "ripple": 4.0, "oil": 13.0, "rust": 8.0}
    masks = {}
    ...
    anchors = {
        "water": hex_rgb("101F29"),
        "ripple": hex_rgb("182B34"),
        "oil": hex_rgb("132224"),
        "rust": hex_rgb("4E3523"),
    }
    flattened_smooth = np.stack(
        [gaussian(np, flattened[..., channel], max(12.0, size / 2048.0 * 52.0)) for channel in range(3)],
        axis=2,
    )
    # The percentages are exact authored coverage masks, but a deep-water BaseColor must
    # remain continuous. Material identities therefore modulate one shared water body
    # instead of becoming isolated opaque islands.
    water_sample = flattened_smooth.mean(axis=(0, 1))
    water_sample = water_sample * 0.18 + anchors["water"] * 0.82
    base = np.broadcast_to(water_sample, (size, size, 3)).copy()
    ripple_weight = masks["ripple"][..., None] * 0.18
    oil_weight = masks["oil"][..., None] * 0.07
    rust_weight = masks["rust"][..., None] * 0.07
    base = base * (1.0 - ripple_weight) + anchors["ripple"] * ripple_weight
    base = base * (1.0 - oil_weight) + anchors["oil"] * oil_weight
    base = base * (1.0 - rust_weight) + anchors["rust"] * rust_weight
    # Tiny, low-saturation oil chroma only; it must not read as a Crystal resource vein.
    oil_chroma = np.stack((meso - 0.5, fine - 0.5, macro - 0.5), axis=2)
    base += masks["oil"][..., None] * oil_chroma * 0.018
    crest = np.clip((wave_field - 0.12) / 0.72, 0.0, 1.0) ** 2
    trough = np.clip((-wave_field - 0.22) / 0.78, 0.0, 1.0) ** 2
    base += crest[..., None] * np.array((0.060, 0.075, 0.086), dtype=np.float32)
    base -= trough[..., None] * np.array((0.012, 0.014, 0.015), dtype=np.float32)
    base += (macro - 0.5)[..., None] * np.array((0.006, 0.009, 0.010), dtype=np.float32)
    ...
    height = (
        ...
        + (macro - 0.5) * 0.006
        + (fine - 0.5) * 0.0018
        + masks["oil"] * (meso - 0.5) * 0.0025
        + masks["rust"] * (0.004 + (fine - 0.5) * 0.004)
    )
    height = gaussian(np, height, max(0.85, radius_scale * 1.8))
    height = np.clip(periodic_component(np, height), 0.42, 0.58)

    delta_x = (np.roll(height, -1, axis=1) - np.roll(height, 1, axis=1)) * 0.5
    delta_y = (np.roll(height, -1, axis=0) - np.roll(height, 1, axis=0)) * 0.5
    strength = 22.0 * (size / 512.0)
    ...
    ao = np.clip(1.0 - concavity * 1.8 - masks["rust"] * 0.022, 0.93, 1.0)
    detail = (
        masks["water"] * (0.12 + 0.12 * fine)
        + masks["ripple"] * (0.40 + 0.20 * fine)
        + masks["oil"] * (0.08 + 0.12 * meso)
        + masks["rust"] * (0.50 + 0.24 * fine)
    )
    smoothness = (
        masks["water"] * (0.82 + 0.065 * fine)
        + masks["ripple"] * (0.88 + 0.055 * fine)
        + masks["oil"] * (0.90 + 0.040 * meso)
        + masks["rust"] * (0.78 + 0.045 * fine)
    )
    smoothness = np.clip(smoothness, 0.78, 0.94)
    mask_rgba = np.stack((metallic, ao, np.clip(detail, 0.0, 1.0), np.clip(smoothness, 0.0, 1.0)), axis=2)

    base_path = args.asset_root / "T_Terrain_DeepWater_BaseColor.png"
    normal_path = args.asset_root / "T_Terrain_DeepWater_Normal.png"
    mask_path = args.asset_root / "T_Terrain_DeepWater_Mask.png"
    height_path = args.asset_root / "T_Terrain_DeepWater_Height.png"
    ...
    tiling.save(args.qa_root / "QA_Terrain_DeepWater_Tiling4x4.png", format="PNG", optimize=True)
...
def build_blender(args: argparse.Namespace):
    ...
    material = bpy.data.materials.new("MAT_Terrain_DeepWater_Golden_Preview")
    ...
    base = image_node("BaseColor_sRGB", "T_Terrain_DeepWater_BaseColor.png", 330, "sRGB")
    normal = image_node("Normal_Linear", "T_Terrain_DeepWater_Normal.png", 70, "Non-Color")
    packed = image_node("URP_Mask_Linear", "T_Terrain_DeepWater_Mask.png", -190, "Non-Color")
    height = image_node("Height16_Linear", "T_Terrain_DeepWater_Height.png", -450, "Non-Color")
    ...
    normal_map.inputs["Strength"].default_value = 1.05
    ...
    light("Key_Area", "AREA", 820.0, (2.5, -3.5, 7.5), (1.0, 0.84, 0.68), 2.4)
    light("Fill_Area", "AREA", 360.0, (-4.5, 1.5, 5.0), (0.56, 0.64, 0.68), 3.2)
    light("Rim_Area", "AREA", 520.0, (4.0, 4.0, 6.0), (0.66, 0.72, 0.74), 2.0)
    ...
    scene.render.filepath = str(args.qa_root / "QA_Terrain_DeepWater_DefaultOrtho.png")
    ...
    scene.render.filepath = str(args.qa_root / "QA_Terrain_DeepWater_PBRCheck.png")
    bpy.ops.render.render(write_still=True)

    generator_text = bpy.data.texts.new("generate_deepwater.py")
    generator_text.write(Path(__file__).read_text(encoding="utf-8"))
    notes = bpy.data.texts.new("README_DeepWater_Golden.txt")
    notes.write(
        "Waste City DeepWater terrain first-pass material sample.\n"
        "User-approved AI concept informed the water palette and ripple language; seed 813246.\n"
    ...
    for image in bpy.data.images:
        if image.source == "FILE":
            image.pack()
    blend_path = args.source_root / "DeepWater_Golden_Generator.blend"
