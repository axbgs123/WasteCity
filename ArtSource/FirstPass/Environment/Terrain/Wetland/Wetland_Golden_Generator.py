...
def make_ora(np, Image, source_root: Path, base_rgb, masks):
    ...
    for index, name in enumerate(("mud", "water", "dry", "sludge", "root", "transition")):
    ...
    ora_path = source_root / "Wetland_Golden_Master.ora"
    stack_layers = []
    ...
    stack_xml = (
        ...
        f'<image version="0.0.1" w="{size}" h="{size}" name="Wetland Golden Master">\n'
        '  <stack name="Wetland Golden Source">\n    '
        + "\n    ".join(stack_layers)
        + "\n  </stack>\n</image>\n"
    )
...
def generate_maps(args: argparse.Namespace):
    ...
    if args.concept is None or not args.concept.is_file():
        raise FileNotFoundError("--concept must point to the user-approved Wetland concept image")
    ...
    categories["mud"] = available

    radius_scale = size / 2048.0
    blur_radius = {"mud": 5.5, "water": 8.0, "dry": 6.0, "sludge": 4.0, "root": 1.6, "transition": 7.0}
    masks = {}
    ...
    height_layers = {
        "mud": 0.480 + 0.018 * independent_macro - 0.010 * basin_boundary,
        "water": 0.405 + 0.006 * basin_value + 0.004 * independent_fine,
        "dry": 0.545 + 0.028 * independent_macro + 0.015 * np.maximum(material_relief, 0.0),
        "sludge": 0.435 + 0.012 * independent_macro,
        "root": 0.530 + 0.024 * np.maximum(material_relief, 0.0) + 0.012 * independent_fine,
        "transition": 0.515 + 0.022 * independent_macro,
    }
    height = np.zeros((size, size), dtype=np.float32)
    for name, values in height_layers.items():
        height += masks[name] * values
    height += material_relief * 0.009 + (independent_fine - 0.5) * 0.003
    ...
    ao = np.clip(1.0 - concavity * 3.6 - masks["root"] * 0.055 - basin_boundary * masks["mud"] * 0.035, 0.72, 1.0)
    detail = (
        masks["mud"] * (0.34 + 0.24 * independent_fine)
        + masks["water"] * (0.08 + 0.13 * independent_fine)
        + masks["dry"] * (0.49 + 0.26 * independent_fine)
        + masks["sludge"] * (0.24 + 0.22 * independent_fine)
        + masks["root"] * (0.70 + 0.27 * independent_fine)
    ...
    base_path = args.asset_root / "T_Terrain_Wetland_BaseColor.png"
    normal_path = args.asset_root / "T_Terrain_Wetland_Normal.png"
    mask_path = args.asset_root / "T_Terrain_Wetland_Mask.png"
    height_path = args.asset_root / "T_Terrain_Wetland_Height.png"
    ...
    tiling.save(args.qa_root / "QA_Terrain_Wetland_Tiling4x4.png", format="PNG", optimize=True)
...
def build_blender(args: argparse.Namespace):
    ...
    material = bpy.data.materials.new("MAT_Terrain_Wetland_Golden_Preview")
    ...
    base = image_node("BaseColor_sRGB", "T_Terrain_Wetland_BaseColor.png", 330, "sRGB")
    normal = image_node("Normal_Linear", "T_Terrain_Wetland_Normal.png", 70, "Non-Color")
    packed = image_node("URP_Mask_Linear", "T_Terrain_Wetland_Mask.png", -190, "Non-Color")
    height = image_node("Height16_Linear", "T_Terrain_Wetland_Height.png", -450, "Non-Color")
    ...
    scene.render.filepath = str(args.qa_root / "QA_Terrain_Wetland_DefaultOrtho.png")
    ...
    scene.render.filepath = str(args.qa_root / "QA_Terrain_Wetland_PBRCheck.png")
    bpy.ops.render.render(write_still=True)

    generator_text = bpy.data.texts.new("generate_wetland.py")
    ...
    for image in bpy.data.images:
        if image.source == "FILE":
            image.pack()
    blend_path = args.source_root / "Wetland_Golden_Generator.blend"
