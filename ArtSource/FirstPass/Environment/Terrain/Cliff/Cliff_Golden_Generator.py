...
def make_ora(np, Image, source_root: Path, base_rgb, masks):
    ...
    for index, name in enumerate(("strata", "crack", "dust", "rubble", "mineral")):
    ...
    ora_path = source_root / "Cliff_Golden_Master.ora"
    stack_layers = []
    ...
    stack_xml = (
        ...
        f'<image version="0.0.1" w="{size}" h="{size}" name="Cliff Golden Master">\n'
        '  <stack name="Cliff Golden Source">\n    '
        + "\n    ".join(stack_layers)
        + "\n  </stack>\n</image>\n"
    )
...
def generate_maps(args: argparse.Namespace):
    ...
    if args.concept is None or not args.concept.is_file():
        raise FileNotFoundError("--concept must point to the user-approved Cliff material/module reference board")

    with Image.open(args.concept) as concept_image:
        concept_image = concept_image.convert("RGB")
        # Panel 1 of the approved 4x2 board is the material close-up. The other
        # panels are modeling references and must not be baked into the texture.
        panel_width = max(1, round(concept_image.width * 0.249))
        panel_height = max(1, round(concept_image.height * 0.499))
        concept_image = concept_image.crop((0, 0, panel_width, panel_height))
    ...
    target_luminance = np.clip(
        0.34 + (luminance - broad_light) * 0.82 + (broad_light - broad_light.mean()) * 0.10,
        0.10,
        0.62,
    )
    ...
    blur_large = gaussian(np, luminance, max(5.0, size / 1024.0 * 64.0))
    ...
    independent_macro = fractal_noise(np, size, [(6, 0.50), (11, 0.31), (19, 0.19)], SEED + 1)
    independent_fine = fractal_noise(np, size, [(47, 0.43), (89, 0.34), (157, 0.23)], SEED + 201)
    fracture, plate_value = periodic_voronoi(np, size, 9, SEED + 501)
    axis = np.arange(size, dtype=np.float32) / size
    yy, xx = np.meshgrid(axis, axis, indexing="ij")
    warp = (independent_macro - 0.5) * 0.38
    strata_wave = (
        np.sin(2.0 * np.pi * (8.0 * yy + warp + 0.18 * np.sin(2.0 * np.pi * 2.0 * xx))) * 0.62
        + np.sin(2.0 * np.pi * (15.0 * yy - warp * 0.45 + 0.10 * np.sin(2.0 * np.pi * 3.0 * xx))) * 0.26
        + np.sin(2.0 * np.pi * (27.0 * yy + independent_fine * 0.12)) * 0.12
    ).astype(np.float32)
    strata_wave = gaussian(np, strata_wave, max(0.8, size / 2048.0 * 2.0))
    vertical_streak = np.abs(np.sin(2.0 * np.pi * (5.0 * xx + independent_macro * 0.20)))

    total = size * size
    available = np.ones((size, size), dtype=bool)
    categories = {}
    # Final coverage fields are entirely periodic. The approved reference supplies
    # palette and shape direction, while the deliverable avoids baking its lighting
    # or non-seamless panel boundaries into production maps.
    mineral_score = vertical_streak * 0.72 + independent_macro * 0.34 - independent_fine * 0.12
    categories["mineral"] = pick_top(np, mineral_score, available, round(total * RATIOS["mineral"]))
    available &= ~categories["mineral"]
    rubble_score = independent_fine * 1.30 + np.abs(strata_wave) * 0.42 + fracture * 0.22
    categories["rubble"] = pick_top(np, rubble_score, available, round(total * RATIOS["rubble"]))
    available &= ~categories["rubble"]
    crack_score = fracture * 2.05 + (1.0 - plate_value) * 0.30 + np.maximum(-strata_wave, 0.0) * 0.46
    categories["crack"] = pick_top(np, crack_score, available, round(total * RATIOS["crack"]))
    available &= ~categories["crack"]
    dust_score = independent_macro * 0.82 - independent_fine * 0.26 + np.maximum(strata_wave, 0.0) * 0.24
    categories["dust"] = pick_top(np, dust_score, available, round(total * RATIOS["dust"]))
    available &= ~categories["dust"]
    categories["strata"] = available

    radius_scale = size / 2048.0
    blur_radius = {"strata": 4.5, "crack": 2.2, "dust": 6.0, "rubble": 1.8, "mineral": 4.0}
    masks = {
    ...
    anchors = {
        "strata": hex_rgb("4C3B2D"),
        "crack": hex_rgb("292521"),
        "dust": hex_rgb("796047"),
        "rubble": hex_rgb("3A3028"),
        "mineral": hex_rgb("5B5146"),
    }
    variation = (
        (independent_macro - 0.5)[..., None] * np.array((0.075, 0.056, 0.038), dtype=np.float32)
        + (independent_fine - 0.5)[..., None] * np.array((0.030, 0.024, 0.018), dtype=np.float32)
        + (plate_value - 0.5)[..., None] * np.array((0.050, 0.040, 0.032), dtype=np.float32)
    )
    base = np.zeros((size, size, 3), dtype=np.float32)
    for name in RATIOS:
        tint = anchors[name] + variation
        base += masks[name][..., None] * tint
    base += strata_wave[..., None] * np.array((0.018, 0.014, 0.010), dtype=np.float32)
    base = np.clip(base, 0.0, 1.0)

    material_relief = np.tanh(
        (independent_macro - 0.5) * 1.8
        + (independent_fine - 0.5) * 0.75
        + strata_wave * 0.62
        - fracture * 0.58
    )
    height_layers = {
        "strata": 0.555 + 0.025 * plate_value + 0.024 * strata_wave + 0.015 * independent_macro - 0.030 * fracture,
        "crack": 0.355 + 0.012 * independent_macro,
        "dust": 0.505 + 0.018 * independent_macro,
        "rubble": 0.500 + 0.070 * np.maximum(material_relief, 0.0) + 0.024 * independent_fine,
        "mineral": 0.475 + 0.010 * vertical_streak,
    }
    ...
    ao = np.clip(1.0 - concavity * 4.4 - masks["crack"] * 0.16 - masks["rubble"] * 0.065, 0.58, 1.0)
    detail = (
        masks["strata"] * (0.58 + 0.27 * independent_fine)
        + masks["crack"] * (0.44 + 0.24 * independent_fine)
        + masks["dust"] * (0.20 + 0.20 * independent_fine)
        + masks["rubble"] * (0.74 + 0.25 * independent_fine)
        + masks["mineral"] * (0.34 + 0.22 * independent_fine)
    )
    smoothness = (
        masks["strata"] * (0.090 + 0.060 * independent_fine)
        + masks["crack"] * (0.055 + 0.040 * independent_fine)
        + masks["dust"] * (0.050 + 0.035 * independent_fine)
        + masks["rubble"] * (0.065 + 0.045 * independent_fine)
        + masks["mineral"] * (0.115 + 0.060 * independent_fine)
    )
    mask_rgba = np.stack((metallic, ao, np.clip(detail, 0.0, 1.0), np.clip(smoothness, 0.0, 1.0)), axis=2)

    # A periodic field may still place an unusually busy fracture exactly at the
    # stored image boundary. Roll every channel together to the quietest valid
    # periodic cut so the 4x4 QA has no conspicuous straight join.
    x_score = (
        np.abs(base - np.roll(base, 1, axis=1)).mean(axis=(0, 2))
        + np.abs(normal - np.roll(normal, 1, axis=1)).mean(axis=(0, 2)) * 0.35
        + np.abs(mask_rgba - np.roll(mask_rgba, 1, axis=1)).mean(axis=(0, 2)) * 0.35
        + np.abs(height - np.roll(height, 1, axis=1)).mean(axis=0)
    )
    y_score = (
        np.abs(base - np.roll(base, 1, axis=0)).mean(axis=(1, 2))
        + np.abs(normal - np.roll(normal, 1, axis=0)).mean(axis=(1, 2)) * 0.35
        + np.abs(mask_rgba - np.roll(mask_rgba, 1, axis=0)).mean(axis=(1, 2)) * 0.35
        + np.abs(height - np.roll(height, 1, axis=0)).mean(axis=1)
    )
    seam_x = int(np.argmin(x_score))
    seam_y = int(np.argmin(y_score))
    base = np.roll(np.roll(base, -seam_x, axis=1), -seam_y, axis=0)
    normal = np.roll(np.roll(normal, -seam_x, axis=1), -seam_y, axis=0)
    mask_rgba = np.roll(np.roll(mask_rgba, -seam_x, axis=1), -seam_y, axis=0)
    height = np.roll(np.roll(height, -seam_x, axis=1), -seam_y, axis=0)
    masks = {name: np.roll(np.roll(value, -seam_x, axis=1), -seam_y, axis=0) for name, value in masks.items()}

    base_path = args.asset_root / "T_Terrain_Cliff_BaseColor.png"
    normal_path = args.asset_root / "T_Terrain_Cliff_Normal.png"
    mask_path = args.asset_root / "T_Terrain_Cliff_Mask.png"
    height_path = args.asset_root / "T_Terrain_Cliff_Height.png"
    ...
    tiling.save(args.qa_root / "QA_Terrain_Cliff_Tiling4x4.png", format="PNG", optimize=True)
    ...
    print(f"PERIODIC_CUT={seam_x},{seam_y}")
...
def build_blender(args: argparse.Namespace):
    ...
    scene["wastecity_asset"] = "Cliff terrain first-pass material sample"
    scene["generator_seed"] = SEED
    scene["texture_resolution"] = args.size
    scene["coverage_ratios"] = "55/20/15/7/3"
    scene["gameplay_truth"] = "none"
    scene["authoring_blender"] = bpy.app.version_string
    scene["preview_renderer"] = "EEVEE"
    scene["approved_concept"] = "Cliff_MaterialAndModules_Approved_AI_Reference_v001.png"
    ...
    material = bpy.data.materials.new("MAT_Terrain_Cliff_Golden_Preview")
    ...
    base = image_node("BaseColor_sRGB", "T_Terrain_Cliff_BaseColor.png", 330, "sRGB")
    normal = image_node("Normal_Linear", "T_Terrain_Cliff_Normal.png", 70, "Non-Color")
    packed = image_node("URP_Mask_Linear", "T_Terrain_Cliff_Mask.png", -190, "Non-Color")
    height = image_node("Height16_Linear", "T_Terrain_Cliff_Height.png", -450, "Non-Color")
    ...
    scene.render.filepath = str(args.qa_root / "QA_Terrain_Cliff_DefaultOrtho.png")
    ...
    scene.render.filepath = str(args.qa_root / "QA_Terrain_Cliff_PBRCheck.png")
    bpy.ops.render.render(write_still=True)

    generator_text = bpy.data.texts.new("generate_cliff.py")
    generator_text.write(Path(__file__).read_text(encoding="utf-8"))
    notes = bpy.data.texts.new("README_Cliff_Golden.txt")
    notes.write(
        "Waste City Cliff terrain first-pass material sample.\n"
        "User-approved AI reference informed the BaseColor rebuild; seed 813417.\n"
    ...
    for image in bpy.data.images:
        if image.source == "FILE":
            image.pack()
    blend_path = args.source_root / "Cliff_Golden_Generator.blend"
